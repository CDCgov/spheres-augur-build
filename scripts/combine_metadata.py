...
def parse_args():
    ...
    parser.add_argument('--origins', required = True, nargs = '+', metavar = "STR",
        help = "Names of origins (order should match provided metadata)")
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(
            "Error. Please check your inputs - there must be the same number of metadata files as origins provided, and there must be more than one of each!")
        sys.exit(2)
                ...
                if column in row:
                    ...
                    if new_value != EMPTY and new_value != existing_value:
                        if existing_value != EMPTY:
                            print(
                                f"[{strain}::{column}] Overwriting {combined_data[ strain ][ column ]} with {new_value}")
